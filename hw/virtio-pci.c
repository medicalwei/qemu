...
static int virtio_memlink_init_pci(PCIDevice *pci_dev)
{
    ...
    vdev = virtio_memlink_init(&pci_dev->qdev);
    if (!vdev) {
        return -1;
    }
    virtio_init_pci(proxy, vdev);
...
static void virtio_memlink_exit_pci(PCIDevice *pci_dev)
{
    VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);

    virtio_pci_stop_ioeventfd(proxy);
    virtio_memlink_exit(proxy->vdev);
    virtio_exit_pci(pci_dev);
}
...
static int virtio_ib_init_pci(PCIDevice *pci_dev)
{
    ...
    vdev = virtio_ib_init(&pci_dev->qdev);
    if (!vdev) {
         return -1;
    }
...
static void virtio_ib_exit_pci(PCIDevice *pci_dev)
{
     VirtIOPCIProxy *proxy = DO_UPCAST(VirtIOPCIProxy, pci_dev, pci_dev);

     virtio_pci_stop_ioeventfd(proxy);
     virtio_ib_exit(proxy->vdev);
     virtio_exit_pci(pci_dev);
}
...
static Property virtio_memlink_properties[] = {
    DEFINE_VIRTIO_COMMON_FEATURES(VirtIOPCIProxy, host_features),
    DEFINE_PROP_HEX32("class", VirtIOPCIProxy, class_code, 0),
    DEFINE_PROP_END_OF_LIST(),
};

static void virtio_memlink_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *k = PCI_DEVICE_CLASS(klass);
    ...
    k->class_id = PCI_CLASS_OTHERS;
    dc->reset = virtio_pci_reset;
    dc->props = virtio_memlink_properties;
}

static TypeInfo virtio_memlink_info = {
    .name          = "virtio-memlink-pci",
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(VirtIOPCIProxy),
...
static void virtio_ib_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *k = PCI_DEVICE_CLASS(klass);
    ...
    k->class_id = PCI_CLASS_OTHERS;
    dc->reset = virtio_pci_reset;
    dc->props = virtio_ib_properties;
}

static TypeInfo virtio_ib_info = {
    .name          = "virtio-ib-pci",
    .parent        = TYPE_PCI_DEVICE,
    .instance_size = sizeof(VirtIOPCIProxy),
    .class_init    = virtio_ib_class_init,
};
